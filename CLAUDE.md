...
**Embedded tap tempo (Format 0 / ZIP only):** `Project.embedTapTempo` (toggled via File menu, persisted in `.midiproj`) adds 4 taps, spaced exactly one beat apart (128 ticks at midi-writer PPQ), starting at tick 66 (half a beat off-beat + 2 ticks), per supported device: `quad-cortex` gets `CC#44 value 127` (QC treats CC#44 as tap and derives BPM from the LAST TWO taps only, no averaging); `ve-500` gets `CC#80` press/release pairs (127, then 0 a quarter-beat later — Boss momentary sources tap on press and need the release between taps). The VE-500 has no fixed tap CC: the singer must configure an ASSIGN with SOURCE=CC#80, TARGET=MASTER:TAP on the device (CC#80 chosen because the profile's assign slots use CC#1–8 and exp pedal CC#11). The off-beat placement is load-bearing: the M-Live B.Beat flushes MIDI output in bursts, so taps sharing an instant with beat-aligned song-start messages arrive bunched and set a random too-high tempo (165 → 172–204 observed); isolated taps timed ±1 BPM. Taps are deliberately NOT latency-compensated (shifting the first tap, clamped at tick 0, would distort the interval); they DO get the liveOffset shift. History: an embedded-MIDI-clock export (0xF8 real-time bytes in the SMF) was tried first because the B.Beat's own clock generator is unreliable, but such files hard-crash the B.Beat (frozen until restart, seen on firmware 5.0.5) — see git history if ever revisiting.
